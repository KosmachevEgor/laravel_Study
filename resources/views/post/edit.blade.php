...
@section('content')
    <form action="{{route('post.update', $post->id)}}" method="post">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" name='image' id="post-image" value="{{$post->image}}" placeholder="Image">
        </div>
        <div class="mb-3">
        ...
        <div>
            <label for="multiple-tag-select">Tags</label>
            <select class="form-select" id="multiple-tag-select" name="tags[]" multiple aria-label="multiple select example">
                @foreach($tags as $tag)
                    <option
